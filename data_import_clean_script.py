...
imports2019 = pd.read_excel('data/data-trade-2019.xlsx', sheet_name='Imports').assign(year=2019).assign(type='import2019')
exports2019 = pd.read_excel('data/data-trade-2019.xlsx', sheet_name='Exports').assign(year=2019).assign(type='export2019')
imports2020 = pd.read_excel('data/data-trade-2020.xlsx', sheet_name='Imports').assign(year=2020).assign(type='import2020')
exports2020 = pd.read_excel('data/data-trade-2020.xlsx', sheet_name='Exports').assign(year=2020).assign(type='export2020')
...
med_trade = tariff_applied.append(imports2019, sort=False)
med_trade = med_trade.append(exports2019, sort=False)
med_trade = med_trade.append(imports2020, sort=False)
med_trade = med_trade.append(exports2020, sort=False)
...
country_blank = set(country_list_med) - set(country_list_wto)
...
med_trade.to_csv('med_trade_test20210325.csv', index=False)
...
#%%%   Special part to procss 2020 Q1-Q3 data received on 20210326

# data = pd.read_excel('data/COVID_Data_trade_l96_quaterly.xlsx', sheet_name='Imports')
# df = data[(data['Year']==2020) & (data['Quater']<3)].groupby(['Country']).agg({'all':sum, 'equipment':sum,'supplies':sum, 'medicines':sum, 'protective':sum}).reset_index()
# df.to_excel('data-tmp-im2020.xlsx')
